...
static int pickleCommandInfo(pickle_t *i, const int argc, char **argv, void *pd) {
	...
	if(!strcmp(rq, "level")) { return pickle_set_result_integer(i, i->level); }
	if(!strcmp(rq, "line"))  { return pickle_set_result_integer(i, i->line); }
...
static int interactive(pickle_t *i, FILE *input, FILE *output) { /**@todo rewrite this a pickle program? */
	...
	for(;;) {
		char clibuf[LINE_SZ];
		const char *prompt = pickle_get_var_string(i, "prompt");
